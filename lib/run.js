...
function eslint() {
  ...
  // move errors to the beginning
  annotations.sort((a, b) => b.annotation_level - a.annotation_level)
  // if warning annotations are disabled, at least put the first 50 in the log
  if (!INPUT_WARNINGS) {
    const warnings = annotations.filter(a => a.annotation_level === 1).slice(0, 50)
    annotations = annotations.filter(a => a.annotation_level > 1)
    warnings.map(m => console.log(`WARN ${m.path}:${m.start_line} ${m.message}`))
  }
  return {
