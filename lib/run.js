...
function eslint() {
  ...
  annotations.sort((a, b) => b.annotationLevel - a.annotationLevel)
  // if warning annotations are disabled, at least put the first 50 in the log
  if (!INPUT_WARNINGS) {
    const warnings = annotations.filter(a => annotation_level === 1).slice(0, 50)
    annotations = annotations.filter(a => annotation_level > 1)
